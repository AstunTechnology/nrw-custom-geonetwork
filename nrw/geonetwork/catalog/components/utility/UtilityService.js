  ...
  module.factory("gnUtilityService", [
    ...
    function (gnPopup, $translate, $location, $rootScope, $timeout, $window, $q, $http) {
      ...
      function displayPermalink(title, url) {
        // Determine the language parameter based on the current URL
            var languageParam = '';
            if (window.location.href.includes('/eng/')) {
                // English language parameter
                languageParam = 'language=eng';
            } else if (window.location.href.includes('/wel/')) {
                // French language parameter
                languageParam = 'language=wel';
            } // Add more conditions for other languages as needed

            // Remove any existing language parameter from the URL
            var urlWithoutLanguageParam = url.replace(/(\?|\&)language=[^&]+/, '');

            // Append the new language parameter to the URL
            var modifiedUrl = urlWithoutLanguageParam + (urlWithoutLanguageParam.includes('?') ? '&' : '?') + languageParam;
