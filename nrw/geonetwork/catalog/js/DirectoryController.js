...
(function () {
  ...
  module.controller("GnDirectoryController", [
	...
	// Fix for NRW: set group to metadata preferred group (if available) or first group in the groups list
	$scope.importData.group =
      ...
      $scope.startImporting = function (asTemplate) {
        ...
        $scope.importData.metadataType = asTemplate
          ...
          : "SUB_TEMPLATE";
	// Fix for NRW: set default group to DataDiscovery
        $scope.importData.group =
         gnConfig["system.metadatacreate.preferredGroup"] || 107 || $scope.groups[0].id;
      };
