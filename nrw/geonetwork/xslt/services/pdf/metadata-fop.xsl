...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  -->
  <!-- the $translations variable is a dump of all the json values but currently in english
  regardless of the url you're requesting it from -->
                ...
                <fo:block margin-left="0pt" margin-right="0pt"
                  ...
                  <fo:table>
                    ...
                    <fo:table-body>
                      ...
                      <xsl:variable name="abstract" select=" if (contains($nodeUrl, 'cym')) then 'Crynodeb' else 'Abstract'"/>
                      <xsl:variable name="extent" select=" if (contains($nodeUrl, 'cym')) then 'Maint gofodol' else 'Spatial Extent'"/>
                      <!-- <xsl:variable name="keywords" select=" if (contains($nodeUrl, 'cym')) then 'Geiriau allweddol' else 'Keywords'"/> -->
                      <xsl:variable name="constraints" select=" if (contains($nodeUrl, 'cym')) then 'Cyfyngiadau cyfreithiol' else 'Constraints'"/>
                      <xsl:variable name="spatialrepresentationtype" select=" if (contains($nodeUrl, 'cym')) then 'Math o Gynrychiolaeth Ofodol' else 'Spatial Representation Type'"/>
                      <!-- <xsl:variable name="format" select=" if (contains($nodeUrl, 'cym')) then 'Fformat' else 'Format'"/> -->
                      <xsl:variable name="updateFrequency" select=" if (contains($nodeUrl, 'cym')) then 'Diweddaru amlder' else 'Update Frequency'"/>
                      <xsl:variable name="uuid" select=" if (contains($nodeUrl, 'cym')) then 'Dynodwr' else 'UUID'"/>
                      <xsl:variable name="updatedOn" select=" if (contains($nodeUrl, 'cym')) then 'Diweddarwyd' else 'Updated On'"/>
                      <xsl:variable name="onTheWeb" select=" if (contains($nodeUrl, 'cym')) then 'Mwy o wybodaeth ar-lein' else 'More online information'"/>
                      <xsl:variable name="show" select=" if (contains($nodeUrl, 'cym')) then 'Dangos' else 'Show'"/>
                      ...
                      <xsl:if test="legalConstraints">
                        <xsl:call-template name="info-rows">
                          <xsl:with-param name="label" select="$constraints"/>
                          <xsl:with-param name="value">
                      ...
                      <xsl:if test="codelist_spatialRepresentationType_text">
                        <xsl:call-template name="info-rows">
                          <xsl:with-param name="label" select="$spatialrepresentationtype"/>
                          <xsl:with-param name="value"
                      ...
                      <xsl:if test="codelist_maintenanceAndUpdateFrequency_text">
                        <xsl:call-template name="info-rows">
                          <xsl:with-param name="label" select="$updateFrequency"/>
                          <xsl:with-param name="value"
                      ...
                      <xsl:call-template name="info-rows">
                        <xsl:with-param name="label" select="$updatedOn"/>
                        <xsl:with-param name="value"
                                        select="changeDate"/>
                      </xsl:call-template>
